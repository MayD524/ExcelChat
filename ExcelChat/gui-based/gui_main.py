...
class ChatGUI: 
    ...
    def layout(self): 
        ...
        self.Window.title(f"{self.name} - Excel Chat")
    ...
    def SelectRoom(self):
        out = UPL.gui.confirm("Select a room", "Click the button for the room you wish to join", self.userData['rooms'])
        self.sendPacket(f"SWITCHROOM:{out}", "SERVER", "SERVER")
    ...
    def notifications(self,message:str, pinged:bool):
        if self.note_settings["soundNotification"] == True:
            if self.note_settings['allowed_notifications'] == "pings" and pinged == True:
                playsound.playsound(self.note_settings['soundNotification_location'])
    
    # function to receive messages 
    def receive(self): 
        temp = None ## in case something were to go wrong use this (messages joining each other)
        while self.running:    
            try: 
                message = client.recv(2048).decode(FORMAT) 
				## for somereason this was being attached to other messages
                
                if "SERVER_ACCEPTED" in message:
                    message = message.replace('SERVER_ACCEPTED', '')
     
				## same as with SERVER_ACCEPTED
    
                ## attempt to fix this later :">
                if f"{self.name} has joined the chat" in message:
                    temp = f"{self.name} has joined the chat"
                    message = message.replace(temp, "")
                    self.textCons.config(state=NORMAL)
                    self.textCons.insert(END, temp+"\n\n")
                    self.textCons.config(state=DISABLED)
                    self.textCons.see(END)
                    
                elif f"{self.name} has joined the room" in message:
                    temp = f"{self.name} has joined the room"
                    message = message.replace(temp, "")
                    self.textCons.config(state=NORMAL)
                    self.textCons.insert(END, temp+"\n\n")
                    self.textCons.config(state=DISABLED)
                ...
                elif f"{self.name} has left the chat" in message:
                    temp = f"{self.name} has left the chat"
                    message = message.replace(temp, "")
                    self.textCons.config(state=NORMAL)
                    self.textCons.insert(END, temp+"\n\n")
                    self.textCons.config(state=DISABLED)
                ...
                if message.startswith("SERVERNAME:"):
                    ...
                    self.HeaderText.set(message)
                    continue
                
                elif message.startswith('USERDATA:'):
                    message       = message.split(':',1)[1]
                    self.userData = eval(message)
                    if self.userData['current_room'] != self.currentRoom:
                        self.currentRoom = self.userData['current_room']
                        self.HeaderText.set(self.currentRoom)
                ...
                match message:
                    ...
                    case _:
                        ...
                        if pinged == False:
                            ...
                            tmp = threading.Thread(target=self.notifications, args=(message, pinged))
                            tmp.start()
                            #self.notifications(message, pinged)
                            self.textCons.insert(END, message+'\n\n', ('highlight',))
                        self.textCons.config(state = DISABLED) 
                        self.textCons.see(END) 
